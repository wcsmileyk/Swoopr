...
def get_passed_criteria(user, category: str) -> set:
    ...
    """
    from training.models import StudentSignoff
    passed = set()
...
def get_category_progress(user) -> dict:
    ...
    for all A–H categories. Uses a single DB query across all categories.
    """
    from training.models import StudentSignoff
    categories = list('abcdefgh')

    # Single query for all sign-offs across all categories
    signoffs = StudentSignoff.objects.filter(
        jump__user=user,
        jump__student_category__in=categories,
    ).values('jump__student_category', 'criteria_passed')

    passed_by_cat = {cat: set() for cat in categories}
    for row in signoffs:
        cat = row['jump__student_category']
        cp = row['criteria_passed']
        for section in ('ground', 'freefall', 'canopy'):
            passed_by_cat[cat].update(cp.get(section, []))

    result = {}
    for cat in categories:
        all_criteria = get_all_general_criteria(cat)
        total = sum(len(v) for v in all_criteria.values())
        passed = passed_by_cat[cat]
        passed_count = sum(
