...
# ---------------------------------------------------------------------------
# Dropzone → organizations.Dropzone
# Aircraft → aircraft.Aircraft
# StudentSignoff → training.StudentSignoff
# PendingInstructorRequest → training.PendingInstructorRequest
# ---------------------------------------------------------------------------
...
class Jump(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='jumps')
    ...
    dropzone = models.ForeignKey(
        'organizations.Dropzone', on_delete=models.PROTECT, related_name='jumps'
    )
    aircraft = models.ForeignKey(
        'aircraft.Aircraft', on_delete=models.PROTECT, related_name='jumps',
    ...
    # DZ manifest link (optional — bridges personal log to DZ-authoritative record)
    load_slot = models.OneToOneField(
        'dz.LoadSlot', on_delete=models.SET_NULL,
        null=True, blank=True, related_name='jump',
    ...
    def renumber_for_user(cls, user):
        ...
        try:
            ...
            start = 1
        jumps = list(
            cls.objects.filter(user=user).order_by('date', 'created_at').only('pk', 'jump_number')
        )
        for i, jump in enumerate(jumps, start=start):
            jump.jump_number = i
        if jumps:
            cls.objects.bulk_update(jumps, ['jump_number'])
...
class InstructorRate(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='instructor_rates')
